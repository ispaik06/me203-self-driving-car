...
X = tf.compat.v1.placeholder(tf.float32, [None, IMG, IMG, 3], name="X")
...
opt  = tf.compat.v1.train.AdamOptimizer(args.lr).minimize(loss)
...
with tf.compat.v1.Session(config=cfg) as sess:
    ...
    # --- TensorBoard writer 설정 (NEW) ---
    logdir = "./logs_lab8"  # 폴더 이름은 마음대로, 나중에 tensorboard --logdir 에서 사용
